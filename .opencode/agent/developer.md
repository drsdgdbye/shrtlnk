...
3. Прогони проверки из брифа: `gofmt`, `go vet ./...`, `go build ./...`, `go test ./...`; для
   конкурентного кода — `go test -race ./...`. Не пропускай упавшие проверки. Запускай их из
   корня репозитория, по одной команде за вызов, без `cd`.
