...
2. Посмотри снимок: `git diff --cached`. Проверь, что изменены только разрешённые брифом файлы.
3. По каждому критерию выполни проверку сам: команда из брифа или собственный сценарий.
...
4. Ищи то, что не попало в дифф: пропущенные требования, лишнее поведение, регрессии, нарушения
   контракта, необработанные ошибки, негативные и граничные сценарии.
5. Посчитай хеш кандидата: `.pipeline/tools/candidate-hash.sh`. Если после этого индекс или
   дерево изменится, проверка недействительна — сообщи лиду.
6. Запиши вердикт: `.pipeline/verdicts/<task_id>.md` (человекочитаемый) и
   `.pipeline/verdicts/<task_id>.json` (машинный; поля — по шаблону
   `.pipeline/templates/verdict.md`).
...
вердикта. Файлы вердикта — единственный источник для приёмки и коммита.
