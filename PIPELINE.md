...
| любое активное | таймаут 60 мин | — | TIMED_OUT → ESCALATE |
...
- Попыток на задачу: 3 (первая + две доработки). Счётчик ведёт плагин в `.pipeline/state`;
  смена агента его не сбрасывает.
- Таймаут диспатча: 60 минут. Лид отменяет зависшего агента (`paseo_cancel_agent`), фиксирует
  `TIMEOUT` в state и эскалирует.
...
брифа — нет диспетча», «нет утверждённого дизайна — нет брифа», бюджет попыток, commit только
по актуальному PASS. На дисциплине ролей: GAP-гейт, watchdog, маршрутизация находок, качество
брифа и вердикта. Эти пробелы названы, а не молчаливы.
